...
export const selectMove = (props: { characterId: number; moveId: number }) =>
  createSelector(selectCharactersState, (state: FrameDataState) =>
    state.moves.find(
      (move) =>
        move.characterId === props.characterId && move.id === props.moveId
    )
  );
